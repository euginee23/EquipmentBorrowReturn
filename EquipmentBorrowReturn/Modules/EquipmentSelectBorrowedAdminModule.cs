...
{
    class EquipmentSelectBorrowedAdminModule
    {
        public static void EquipmentBorrowSelect(DataGridView borrowedequipmentdatagrid, TextBox transactionID, TextBox equipmenttxt, TextBox quantitytxt, TextBox borrowertxt, TextBox personneltxt, TextBox dateborrowedtxt, TextBox scheduletxt, DataGridViewCellEventArgs e)
        {
            int rowIndex = e.RowIndex;
            if (rowIndex < 0) return;

            // Get the selected row data
            DataGridViewRow row = borrowedequipmentdatagrid.Rows[rowIndex];
